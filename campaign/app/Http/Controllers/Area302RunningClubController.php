...
class Area302RunningClubController extends Controller
{
    ...
    {
        $this->number = 1;
        $this->apply_type = CommonApplyConst::APPLY_TYPE_AREA_302_RUNNING_CLUB;
        $this->apply_service = new CommonApplyService($this->apply_type, $this->number);

        if ($this->checkErrorViewRedirect()) {
            Redirect::route('area-302-running-club.outsidePeriod')->send();
        }
    }
    ...
    {
        return view('area_302_running_club.index');
    }
    ...
    {
        return view('area_302_running_club.complete');
    }
    ...
    public function store(MinatoRunnersBaseRequest $request)
    {
        ...
        try {
            ...
            DB::commit();
            Redirect::route('area-302-running-club.complete')->send();
    ...
    private function sendThankYouMail(MinatoRunnersBaseRequest $request)
    {
        ...
        Mail::send('emails.area_302_running_club.thankYouMail', $data, function ($message) {
            $message->to($this->email)
                ->from('info@newbalance-campaign.jp')
                ->bcc("fujisawareon@yahoo.co.jp")
                ->subject('8/17（土）イベントへのお申込みが完了しました。');
        });
    }
    ...
    private function sendReportMail(MinatoRunnersBaseRequest $request)
    {
        ...
        Mail::send('emails.area_302_running_club.reportMail', $data, function ($message) {
            $message->to("nbrun@fluss.co.jp")
                ->from('info@newbalance-campaign.jp')
                ->bcc("fujisawareon@yahoo.co.jp")
                ->subject('「Area 302 running club」のイベントに申し込みがありました');
        });
    }
    ...
    public function outsidePeriod()
    {
        ...
        return view('area_302_running_club.notApplicationPeriod', compact('checkMessage'));
    }
    ...
    private function checkErrorViewRedirect()
    {
        ...
        if (\Route::currentRouteName() ==  'area-302-running-club.outsidePeriod') {
            return false;
        }
