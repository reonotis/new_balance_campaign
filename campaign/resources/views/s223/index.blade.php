        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 ">
                    ...
                    <form action="{{route('s223.store')}}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="itemRow">
                            <label class="itemTitle">希望時間</label>
                            <div class="itemContent">
                                <div>
                                    @foreach($choiceTimeList as $key => $val)
                                        <span>
                                            <label>
                                                <input type="radio" class="" name="choice_time" value="{{ $key }}"
                                                    @if(old('choice_time') == $key)
                                                       checked="checked"
                                                    @endif
                                                    @if($val['apply_limit'])
                                                        disabled = "disabled"
                                ...
                                <p class="support-msg">※選択できない時間帯は、申込数が上限に達しています。</p>
                            </div>
                        </div>
                        <div class="itemRow">
